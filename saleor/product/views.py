...
#<added by chhunneng "def requests" and "def added_requests">
@login_required
def requests(request):
    ...
    if form.is_valid():
        form.save()
        messages.success(
            request, pgettext("Request success message", "Product successfully request.\nThank for using our service.")
        )
        form=RequestForm()
    else:
        form=RequestForm()
    ctx={'form':form}
    return TemplateResponse(request, 'product/request.html',ctx)
def added_requests(request):
    form=RequestForm()
    ctx={'form':form}
    return TemplateResponse(request, 'product/request.html',ctx)
#</added by chhunneng "def requests" and "def added_requests">