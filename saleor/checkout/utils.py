...
def get_checkout_context(
    ...
    if checkout.shipping_method != None and checkout.shipping_method.type == 'percentage':
        checkout_total = checkout.get_total_with_type_percentage(discounts,taxes)
        shipping_price = checkout.get_shipping_price_with_type_percentage(discounts,taxes)
        checkout.save()
    else:
        checkout_total = checkout.get_total(discounts, taxes)
...
def _process_shipping_data_for_order(checkout,discounts,taxes):
    ...
    if checkout.shipping_method.type == "percentage":
        shipping_price_net = checkout.get_shipping_price_with_type_percentage(discounts, taxes)
        total = checkout.get_total_with_type_percentage(discounts, taxes)
    else:
        total = checkout.get_total(discounts, taxes)
        shipping_price_net = checkout.get_shipping_price(taxes)
    # ADD>
    ...
    return {
        ...
        "shipping_price": shipping_price_net,
        "weight": checkout.get_total_weight(),
...
def prepare_order_data(
    *, checkout: Checkout, tracking_code: str, discounts, taxes
    ...
    order_data.update(_process_shipping_data_for_order(checkout,discounts,taxes))
